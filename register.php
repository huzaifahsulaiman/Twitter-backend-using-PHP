...
$username = htmlentities($_REQUEST["username"]); // REQUEST -- catching
$password = htmlentities($_REQUEST["password"]);
$email = htmlentities($_REQUEST["email"]);
$fullname = htmlentities($_REQUEST["fullname"]);

// secure password -- from given password it will create some kind of random letter to password
...
if ($result) {
    ...
    // store all class in $email var means create a new email and save everything in var $email
    $email = new email();

    // store generated token in $token var
    $token = $email->generateToken(20);

    // save information in 'emailTokens' table -- emailTokens is in access.php
    $access->saveToken("emailTokens", $user["id"], $token);

    // emailing information for confirmation
    $details = array();
    $details["subject"] = "Email confirmation on Twitter";
    $details["to"] = $user["email"];
    $details["fromName"] = "Huzaifah Sulaiman";
    $details["fromEmail"] = "huzaifah1008@gmail.com";

    // access template file -- to be shown on email
    $template = $email->confirmationTemplate();

    // replace {token} from confirmationTemplate.html by $token generated and store all content in $template var
