...
if ( empty($_REQUEST["username"]) || empty($_REQUEST["password"])) {
    //passing back to appVC under array
...
$secure_password = $user["password"]; //password we take from database
$salt = $user["salt"];


// 4.2 Check if entered passwords match with password from database
if ($secure_password == sha1($password . $salt)) { //sha1 is used in securing the password, combining $password entered and $salt from database(which is assigned when signing up) 
...
// STEP 6. JSON data (giving back all info to user)
echo json_encode($returnArray);
