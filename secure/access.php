...
class access {
    ...
    public function registerUser($username, $password, $salt, $email, $fullname) {
        ...
        // bind 5 param of type string("sssss") to be placed in $sql command
    ...
    public function selectUser($username) {
        ...
        // sql command -- getting from username's details 
        $sql = "SELECT * FROM users WHERE username='".$username."'"; //* -- means ALL

        // put result we got from $sql to $result var
    ...
    public function saveToken($table, $id, $token) {
        ...
        $statement->bind_param("is", $id, $token); // --- i for integer(id) s for string (token)
    ...
    function getUserID($table, $token) {
        ...
        //declare as array because it has many values
        $returnArray = array();
    ...
    public function getUser($username) {
        ...
        // execute / query $sql ,,, conn has all localhost connection declared in connect() function
        $result = $this->conn->query($sql);
        ...
        if ($result != null && (mysqli_num_rows($result) >= 1)) {
            ...
            // if there's value in $row then assign everything in $returnArray
            if (!empty($row)) {
                $returnArray = $row;
            }
        }
    ...
    public function selectUserViaEmail($email) {
        ...
        // sql command --- * means all detail in the row
    ...
    public function selectPosts($id) {
        ...
        // each time append to $returnArray generate new row one by one when it is found
    ...
    public function selectUsers($word, $username) {
        ...
        //Below: searching data without getting all users
        // sql statement to be executed if not entered word
        //$sql = null;

        // if word entered alter sql statement for wider search
       // if (!empty($word)) {
            //$sql = "SELECT id, username, email, fullname, ava FROM users WHERE NOT username = '".$username."' AND ( username LIKE ? OR fullname LIKE ? )";
       // }else{
            //$sql = "SELECT id, username, email, fullname, ava FROM users WHERE username = '".$username."' ";
       // }
        ...
        if (!empty($word)) {
            $word = '%' . $word . '%'; // %bob%
            $statement->bind_param("ss", $word, $word); //username OR fullname
        }
